/****** Object: View [dbo].[VIEW_WORKINGSTATISTIC]   Script Date: 2013/09/21 08:48:16 ?.? ******/
USE [andicator];
GO
SET ANSI_NULLS ON;
GO
...
SELECT     dbo.USERS.NAME + ' ' + dbo.USERS.FAMILY + ' ( ' + dbo.USERS.USERNAME + ')' AS USERNAME,
                      ...
                      AS RATE_DEC_ORAL_ALL,
                          ...
                          (SELECT     COUNT(0) AS EXPR1
                             FROM         (SELECT     USERTREEID, PRICE, ID AS CHEQUEID,ARCHIVE,
                                                     (SELECT     dbo.CHEQUE.PRICE - SUM(PRICE) AS EXPR1
                                                        ...
                                                        WHERE     (CHEQUEID = dbo.CHEQUE.ID)) AS REMAINED,REFFERDATE
                                                     FROM         dbo.CHEQUE) AS R_1
                             WHERE  (ARCHIVE IS NULL OR ARCHIVE = 0)AND (R_1.REFFERDATE < GETDATE() - CAST('1900-01-04' AS DATETIME)) AND     (PRICE <> REMAINED) AND (USERTREEID = dbo.USERTREE.ID) AND EXISTS
                          ...
                          (SELECT     COUNT(0) AS EXPR1
                             ...
                             WHERE   (ARCHIVE IS NULL OR ARCHIVE = 0) AND (R.REFFERDATE < GETDATE() - CAST('1900-01-04' AS DATETIME)) AND   (USERTREEID = dbo.USERTREE.ID) AND (NOT EXISTS
                                                       (SELECT     ID, CHEQUEID, RECIEPTBILLNUMBER, ISSUEDATE, PRICE, DESCRIPTION, REGISTERDATE, REGISTRANTUSER
